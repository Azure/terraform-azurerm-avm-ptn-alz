resource "azapi_resource" "management_groups_level_0" {
  ...
  body = {
    properties = {
      details = {
        parent = {
          id = "${coalesce(lookup(var.parent_id_overrides.management_groups, each.value.parent_id, null), "/providers/Microsoft.Management/managementGroups")}/${each.value.parent_id}"
        }
      }
